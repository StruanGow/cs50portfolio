...
def buy():
    ...
    if request.method == "POST":
        ...
        if not is_int(shares_nbr):
            return apology("Fraction not supported", 400)
        if int(shares_nbr) <= 0:
            return apology("Share number cannot be negative number or zero!", 400)
        ...
        if user_cash[0]["cash"] < total_cost:
            return apology("Can't afford that", 400)
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(rows) == 1:
            ...
            hashcode = generate_password_hash(password, method='pbkdf2', salt_length=8)
...
def passwordupdate():
    ...
    if request.method == "POST":
        ...
        if not check_password_hash(rows[0]["hash"], currentpassword):
            ...
            hashcode = generate_password_hash(newpassword, method='pbkdf2', salt_length=8)
